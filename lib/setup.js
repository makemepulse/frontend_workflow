...
const Setup = (function(options) {
  ...
  const _install = function() {
    let cli = spawn('npm', ['install'], { stdio:'inherit', cwd: `${_workflow_path}/${_workflow_name}` })
  ...
  const _commandList = function(options) {
    ...
    Print.log(`
      "scripts": {
        ...
        "browserify:compile": "$(npm bin)/workflow stylus -s"
        "browserify:watch": "$(npm bin)/workflow stylus -s -w"
        "browserify:build": "$(npm bin)/workflow stylus -m"
